...
def DeforumAnimArgs(maxFrames,keyFrames):
    ...
    tot_frames = maxFrames #@param {type:"number"}
    ...
    strength_schedule = "0: (0.5)"#@param {type:"string"}
...
def DeforumArgs(output_path):
    ...
    seed = random.randint(0, 2**32 - 1) #@param
    ...
    n_batch = 1 #@param
    n_samples = 1 #@param
    batch_name = "StableFun_kl" #@param {type:"string"}
    filename_format = "{timestring}_{index}_{prompt}.png" #@param ["{timestring}_{index}_{seed}.png","{timestring}_{index}_{prompt}.png"]
    seed_behavior = "iter" #@param ["iter","fixed","random","ladder","alternate"]
    seed_iter_N = 1 #@param {type:'integer'}
    make_grid = False #@param {type:"boolean"}
    grid_rows = 2 #@param 
    outdir = output_path#@param {type:"string"}
